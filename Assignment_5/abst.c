...
int heightOfTree(aBST tnode){
    if(isEmpty(tnode))          //Height of empty tree is -1
        return -1;
    int size = tnode.size;
    int count = 0 ;      //Maximum height element is present at last index , so iterate it reverse to get height
    
    for(int i = size ; i >= 0 ; i = i/2-1){ //Decrement and go towards top with incrementing count
        count++;
    }
    return count;
}
